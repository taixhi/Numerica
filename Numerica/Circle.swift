...
enum CircleType{
    case light, normal, dark, unavailable
}
class CircleSprite: SKSpriteNode{
    let circleType: CircleType
    let circleColor: UIColor
    let number = 0
    let circle: Circle
    let mainTexture: SKTexture
    init(color: UIColor, size: CGSize, type: CircleType) {
        self.circleType = type
        switch circleType{
        case .light:
            circleColor = UIColor(hex: "#9EEADC")
        case .dark:
            circleColor = UIColor(hex: "#008673")
        case .normal:
            circleColor = UIColor(hex: "#40D5BB")
        default:
            circleColor = UIColor(hex: "#888888")
        }
        self.circle = Circle(size: size)
        texture = view.texture(node: self.circle)!
        super.init(texture: self.mainTexture, color: .clear, size: self.mainTexture.size())    }
...
class Circle: SKShapeNode{
    ...
    init(size: CGSize) {
        self.size = size
        let circle = CGRect(origin: CGPoint.zero, size: size)
        super.init()
        self.path = UIBezierPath(ovalIn: circle).cgPath
    }
